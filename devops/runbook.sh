...
docker build -t agent:v1 --build-arg RUNNER_TOKEN=AADQVX3SFNFZJHXKWVQJMITC6X5WQ --build-arg RUNNER_GITHUB_URL=https://github.com/tamaw/launchk8 --build-arg RUNNER_LABELS= -f agent.Dockerfile .
...
docker run --rm --entrypoint htpasswd registry:2.6.2 -Bbn devops pass123 > secrets/htpasswd
...
kubectl create secret docker-registry registry --docker-server=registry-svc.devops.svc.cluster.local:5000 --docker-username=devops --docker-password=pass123 --docker-email=a@a 
...
echo "10.109.95.179 registry-svc.devops.svc.cluster.local" >> /etc/hosts
...
# connect so that the node gets the docker/config.json file (required)
minikube ssh
docker login registry-svc.devops.svc.cluster.local:5000 --username devops
